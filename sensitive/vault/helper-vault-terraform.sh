...
apt-get install -y \
    ...
    lsb-release \
    tree
# 2. Download and dearmor the HashiCorp GPG key
wget -O- https://apt.releases.hashicorp.com/gpg | gpg --dearmor | tee /usr/share/keyrings/hashicorp-archive-keyring.gpg
# 3. Add the repository to your system sources dynamically
...
# export SHA256=$(sha256sum /opt/vault/plugins/graphql | cut -d ' ' -f1)
